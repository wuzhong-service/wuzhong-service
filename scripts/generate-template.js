...
/** 标签活动：档位对应品规和数量（活动品规带颜色、激励品规无颜色） */
function genPackages() {
  const row = (品牌组, 品牌, 品规名称, d30, d27, d19, d14, d9) => {
    const isActivity = 品牌组.startsWith('组')
    return { 品牌组, 品牌, 类型: isActivity ? '活动品规' : '激励品规', 品规名称, '30-28档数量': d30, '27-20档数量': d27, '19-15档数量': d19, '14-10档数量': d14, '9-1档数量': d9, 更新时间: today() }
  }
  return [
    row('上烟集团', '上烟集团', '中华(硬红)', 25, 20, 15, 8, 5),
    row('上烟集团', '上烟集团', '沪前门(软)', 25, 20, 15, 8, 5),
    row('上烟集团', '上烟集团', '牡丹(软)', 15, 10, 8, 4, 3),
    row('组1', '江苏中烟', '苏烟(软五星红杉树)', 24, 22, 16, 10, 6),
    row('组1', '江苏中烟', '南京(硬炫赫门)', 18, 16, 12, 7, 4),
    row('组2', '江苏中烟', '南京(硬大观园爆冰)', 15, 10, 5, 3, ''),
    row('组2', '江苏中烟', '苏烟(硬五星红中)', 5, 5, 3, 2, ''),
    row('组2', '江苏中烟', '南京(硬紫树)', 18, 13, 7, 4, ''),
    row('组3', '江苏中烟', '南京(硬精品)', 10, 8, 5, 3, 2),
    row('组3', '江苏中烟', '南京(硬十二钗薄荷)', 10, 8, 5, 3, 2),
    row('组3', '江苏中烟', '南京(硬红)', 15, 12, 7, 4, 2),
    row('组4', '江苏中烟', '南京(软九五)', 5, 4, 2, '', ''),
    row('组4', '江苏中烟', '利群(硬新版)', 2, 2, 1, '', ''),
    row('组5', '江苏中烟', '南京(硬十二钗烤烟)', 6, 6, 4, 4, 4),
    row('组5', '江苏中烟', '黄山(硬新一品)', 9, 9, 6, 6, 6),
    row('云南中烟', '云南中烟', '云烟(软珍品)', 22, 18, 12, 7, 4),
    row('云南中烟', '云南中烟', '红塔山(软13mg经典1956)', 25, 20, 15, 8, 5),
    row('福建中烟', '福建中烟', '七匹狼(硬银中支)', 5, 3, 2, '', ''),
    row('福建中烟', '福建中烟', '七匹狼(软灰)', 3, 2, 2, '', ''),
    row('福建中烟', '福建中烟', '七匹狼(纯境)', 6, 4, 3, '', ''),
  ]
}
